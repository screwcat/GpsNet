...
//继承 自  OverlayView 
PopupMarker.prototype = new soso.maps.Overlay();

PopupMarker.prototype.construct = function () {
    /*
    var panes = this.getPanes().overlayLayer; alert(panes);*/
};
...
function PopupMarker(opts) {
    ...
    this.dummyTextNode = document.getElementById('spanMapPopupContentSize');
    this.panes_ = null;
...
PopupMarker.prototype.onAdd = function () {
    ...
    var panes = this.panes_;
    panes.appendChild(this.iconContainer);
    panes.appendChild(this.container_);
    this.container_.parentNode.style.zIndex = 9999;
...
//    this.panes_ = this.getPanes().overlayLayer;
    this.panes_ = this.getPanes().floatPane;
    if (this.isAdd_) {
        this.onAdd();
        this.isAdd_ = false;
    }
...
PopupMarker.prototype.destroy = function () {
...
PopupMarker.prototype.setPosition = function (latlng) {
    try {
        ...
        this.container_.style.left = (pxPos.x - 8) + "px";
        //this.container_.style.top = (pxPos.y - this.size_.height) + "px";
        this.container_.style.top = (pxPos.y - this.size_.height - 10) + "px";
        var iconLeft = 3, iconTop = 10;
        if (this.icon_ == "images/1px.png") {
            iconLeft = 0;
            iconTop = 0;
        }
        this.iconContainer.style.left = (pxPos.x - this.ICON_WIDTH / 2 + iconLeft) + "px";
        //this.iconContainer.style.top = (pxPos.y - this.ICON_HEIGHT) + "px";
        this.iconContainer.style.top = (pxPos.y - this.ICON_HEIGHT + iconTop) + "px";
        //this.latlng_ = latlng;
    } catch (e) { }
};
