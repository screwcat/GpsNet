    ...
    if ($('.grid').attr('singleselect') == 'true') {
        $('.grid tr td').click(function (e) {
            if ($(this).parents('tr').find("td").hasClass('selected')) {
                ...
                $('.grid tr td').parents('tr').find('input[type="box"]').removeAttr('ed');
            } else {
                $('.grid tr td').parents('tr').find("td").removeClass('selected');
                $('.grid tr td').parents('tr').find('input[type="box"]').removeAttr('ed');
                $(this).parents('tr').find("td").addClass('selected');
                $(this).parents('tr').find('input[type="box"]').attr('ed', 'ed');
            }
        });
    } else {
        $('.grid tr td').click(function (e) {
            if (!$(this).hasClass('oper')) {
                if ($(this).parents('tr').find("td").hasClass('selected')) {
                    ...
                    $(this).parents('tr').find('input[type="box"]').removeAttr('ed');
                } else {
                    $(this).parents('tr').find("td").addClass('selected');
                    $(this).parents('tr').find('input[type="box"]').attr('ed', 'ed');
                }
...
function AllLine() {
    if (AllLinestatus == 0) {
        AllLinestatus = 1;
        $("#AllOff").attr('title', '反选');
        $("#AllOff").attr('id', 'AllOn');
        $(".grid :box").attr("ed", true);
        $('.grid tr').find('td').addClass('selected');
        $("#dnd-example :box").attr("ed", true);
        $('#dnd-example tr').find('td').addClass('selected');
    } else {
        AllLinestatus = 0;
        $("#AllOn").attr('title', '全选');
        $("#AllOn").attr('id', 'AllOff');
        $(".grid :box").attr("ed", false);
        $('.grid tr').find('td').removeClass('selected');
        $("#dnd-example :box").attr("ed", false);
        $('#dnd-example tr').find('td').removeClass('selected');
    }
}
///防止重复提交
function SubmitForRC() {
